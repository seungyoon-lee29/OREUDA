...
| **프로세스** | 설계 문서 7편([`docs/01~07`](./docs)) 선행 → 구현 → **리뷰 게이트**(코드리뷰 + 이종 모델 적대 리뷰) → E2E 스모크 → 프로덕션 반영. AI 에이전트 협업 워크플로를 직접 설계·운영 ([`AGENTS.md`](./AGENTS.md), [`docs/agents/`](./docs/agents)) |
...
- ✅ 지도: 서울 19산 42코스 실경로 · 산 탭 → 코스선 · 완등 색칠
