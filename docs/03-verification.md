...
| `accuracy` | GPS 정확도 불량 — 신뢰도 하향 | 클라 보고 `accuracy_m` > 100 (100 == 통과) |
...
| GPS 정확도 불량 | `accuracy > 100m`는 클라 소프트 확인('그래도 인증') 후 제출 + 서버 `accuracy` flag ([04 §4.1](./04-client-architecture.md)) — 막다른 차단 없음(관대 정책 §3 정합) | 정확도 값 자체도 클라 주장값 |
...
| 단위 버그(무증상 전부 통과) | geography 캐스팅 + 경계값 테스트 + 클라 가드: `verify_radius_m` zod `min(10).max(2000)` (로컬-서버 거리 괴리 Sentry 이벤트는 [v1] — v0 Sentry 미도입, HANDOFF rank9 블록) | — |
