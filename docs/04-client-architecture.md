...
- **코스선 노출** (서버는 무시, [02 §5.1](./02-backend-spec.md)): **완등(verified)·대기(pending) 코스선은 상시 색칠**(2026-07-19 — 누적 채색이 제품 핵심 가치라 산 선택과 무관하게 항상 렌더, 소요시간 라벨은 선택된 산에서만). **미완등 코스선만 산 선택 게이트**: 산 마커 탭 → 해당 산의 코스선 노출(+진행 중 등반 코스는 유지), 시트 close = 해제. 산 마커는 줌 무관 상시 표시. (구 줌 히스테리시스 z≥11.5/<10.5는 2026-07-07 상호작용 재설계로 대체.)
