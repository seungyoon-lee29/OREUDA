...
2. **[해결] 고줌 checkpoint 마커 제거** — 정상점 공유로 핀 겹침 + 마커가 코스선 끝 덮어 라인 탭(미리보기)을 가로챔 → 제거. 이제 코스 선택은 라인 탭이 유일 경로, 미리보기(라인 강조+카메라 fit+시트) 시뮬 검증 통과(`scratchpad/p1-09-preview.png`). 우상단 톱니는 expo-dev-client 오버레이(프로덕션 없음, 무관).
