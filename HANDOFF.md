# 핸드오프 — 2026-07-19 (완등 코스 상시 색칠 — README 핵심 가치 실물화)

## 방금 한 것
- **사용자 지적**: README 핵심 피치("코스 단위로 색칠되는 나만의 완등 지도")가 실제 앱에 없음 — 코스선이 산 탭 시에만 노출돼 완등 코스도 평소엔 안 보였음(스크린샷에도 부재).
- **수정(루트)**: `index.tsx` visibleCourses에 verified/pending 상시 포함(미완등만 산-탭 게이트 유지), 소요시간 라벨은 선택 산으로 한정(전역 라벨 노이즈 방지). 리뷰 HIGH 반영: `useVerifiedSet` useMemo 참조 안정화(colored.ts — 매 렌더 새 Set이면 지도 오버레이 전체 재커밋).
- **게이트**: code-reviewer 1패스(HIGH 1 반영·MEDIUM 판단 종결) · tsc 0 · 31/31 · 시뮬 실렌더(무선택 상태에서 인왕산+청계산 완등 코스 글로우 색칠 확인).
- **README**: map.png 교체(무선택 상태 완등 2산 색칠 히어로 샷) + 상태 문구 "완등 코스 상시 색칠". docs/04 §7 노출 규칙 갱신.
- 백로그 추가: selectCourse 타 산 탭 시 시트 이전 산 플리커(v0 노이즈) · 05 §3.2 미완등 색 스펙-구현 드리프트 문서 정리.

## 다음 할 것
1. **실기기 런타임 검증**(티켓 01) — 변동 없음.
